...
export const handler = async (event: OnEventRequest, context: Context): Promise<OnEventResponse> => {
  ...
  const resourcePropertiesPartial = resourceProperties as Partial<CloudFormationCustomResourceResourcePropertiesCommon>;
  ...
  while (true) {
    attemptNo += 1;
    console.log(`Attempt: ${attemptNo}. Time Remaining: ${timeRemaining / 1000}s`);
      ...
      switch (command.Status!) {
        ...
        case 'InProgress':
          ...
          if (timeRemaining > SLEEP_MS) {
            console.log(`Instance ${instanceId} not ready: 'InProgress'. Sleeping: ${SLEEP_MS / 1000}s`);
