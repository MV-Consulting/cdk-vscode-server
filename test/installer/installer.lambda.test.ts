...
describe('installer lambda', () => {
  ...
  test('should throw error for failed SSM command', async () => {
    ...
    await expect(handler(event, mockContext))
      .rejects
      .toThrow('SSM Document test-document on EC2 instance i-1234567890 failed');
  });
  ...
  test('should handle command in progress and timeout', async () => {
    ...
    await expect(handler(event, mockContext))
      .rejects
      .toThrow('SSM Document test-document on EC2 instance i-1234567890 timed out while lambda in progress');
  });
});
